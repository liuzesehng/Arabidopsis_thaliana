...
# 数据预处理
## 提取CHG_1到CHG_226列和温度信息（第1列）
chg_cols <- grep('^CHG_[0-9]+$', colnames(data), value = TRUE)
chg_values <- as.matrix(data[, chg_cols])
temperature <- data[, 1]

# 执行PCA（如果数据维度允许）
## 基于其他列创建更多特征用于PCA
feature_cols <- c("Tem..", "Lat", "Long", "alt.m", chg_cols)
...
# 创建热图数据 - 转置矩阵使样本成为列
## 创建热图数据 - CHG_1到CHG_226，样本为列

heatmap_data <- t(chg_values[ordered_indices, ])
colnames(heatmap_data) <- ordered_indices
rownames(heatmap_data) <- chg_cols

# 自动调整热图和PDF尺寸
num_samples <- ncol(heatmap_data)
num_chg <- nrow(heatmap_data)
# 每个样本宽度0.25cm，最小宽度12cm，最大30cm
pdf_width <- min(max(num_samples * 0.25, 12), 30)
# 每个CHG高度0.18cm，最小高度8cm，最大20cm
pdf_height <- min(max(num_chg * 0.18, 8), 20)
...
# 保存为PDF
pdf_file <- "rca_CHG_pca_heatmap.pdf"
