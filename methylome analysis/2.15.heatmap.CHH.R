...
# 数据预处理
## 提取CHH_1到CHH_226列和温度信息（第1列）
chh_cols <- grep('^CHH_[0-9]+$', colnames(data), value = TRUE)
chh_values <- as.matrix(data[, chh_cols])
temperature <- data[, 1]

# 执行PCA（如果数据维度允许）
## 基于其他列创建更多特征用于PCA
feature_cols <- c("Tem..", "Lat", "Long", "alt.m", chh_cols)
...
# 创建热图数据 - 转置矩阵使样本成为列
## 创建热图数据 - CHH_1到CHH_226，样本为列

heatmap_data <- t(chh_values[ordered_indices, ])
colnames(heatmap_data) <- ordered_indices
rownames(heatmap_data) <- chh_cols

# 自动调整热图和PDF尺寸
num_samples <- ncol(heatmap_data)
num_chh <- nrow(heatmap_data)
# 每个样本宽度0.25cm，最小宽度12cm，最大30cm
pdf_width <- min(max(num_samples * 0.25, 12), 30)
# 每个CHH高度0.18cm，最小高度8cm，最大20cm
pdf_height <- min(max(num_chh * 0.18, 8), 20)
...
# 保存为PDF
pdf_file <- "rca_CHH_pca_heatmap.pdf"
