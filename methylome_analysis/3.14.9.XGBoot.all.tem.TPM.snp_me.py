...
# 读取原始数据
df_all = pd.read_csv('../RCA/Alt.all.snp_meth.filtered.tsv', sep='\t')
...
for filter_val in [10, 16, 22]:
    ...
    # 创建目录（如果不存在），区分不同数据集
    output_dir = f"TPM_4.5_all_{filter_val}"
    ...
    print(f"[{filter_val}] Best R² score: ", study.best_value)
    
    # 将最优参数写入文件
    with open(f'{output_dir}/{name_TPM}_optimization_results.txt', 'w', encoding='utf-8') as f:
        f.write(f"=== {name_TPM} 优化结果 (Group {filter_val}) ===\n\n")
        f.write(f"最优参数:\n")
    ...
    plt.title(rf'Total expression level of Rca ({filter_val:g}$^\circ$C)', fontsize=TITLE_FONT_SIZE)
