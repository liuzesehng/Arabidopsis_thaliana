...
# 对残差进行正态性检验
temp_aov <- aov(β.. ~ Temperature, data = data_stats)
cat("\n=== 残差正态性检验 ===\n")
...
cat("\n=== 方差齐性检验 (Levene's test) ===\n")
levene_result <- leveneTest(β.. ~ Temperature, data = data_stats)
print(levene_result)

# Bartlett检验 (对正态分布数据)
cat("\n=== 方差齐性检验 (Bartlett's test) ===\n")
bartlett_result <- bartlett.test(β.. ~ Temperature, data = data_stats)
print(bartlett_result)

# 使用 Welch's t-test (适用于方差不齐的情况)
cat("\n=== Welch's t-test (韦尔奇 t 检验) ===\n")
welch_result <- t.test(β.. ~ Temperature, data = data_stats, var.equal = FALSE)
print(welch_result)
...
wilcox_result <- wilcox.test(β ~ Temperature, data = data_stats, exact = FALSE)
print(wilcox_result)
