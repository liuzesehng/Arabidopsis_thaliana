...
# 创建箱线图
p <- ggplot(data, aes(x = Temperature, y = TPM, fill = Temperature)) +
  # 添加箱须末端标记（上下边缘）
  ...
  # 设置y轴范围
  scale_y_continuous(limits = c(0, max(data$TPM, na.rm = TRUE) * 1.4))

# 显示图形
print(p)

# 保存图形
ggsave("temperature_tpm_boxplot.pdf", plot = p, width = 12, height = 9, dpi = 1200)
...
summary_stats <- data %>%
  ...
  summarise(
    ...
    q25 = quantile(TPM, 0.25, na.rm = TRUE),
    q75 = quantile(TPM, 0.75, na.rm = TRUE),
    .groups = 'drop'
  )
...
shapiro_results <- data %>%
  group_by(Temperature) %>%
  summarise(
    shapiro_p = shapiro.test(TPM)$p.value,
    .groups = 'drop'
  )
print(shapiro_results)

# 对残差进行正态性检验
temp_aov <- aov(TPM ~ Temperature, data = data)
cat("\n=== 残差正态性检验 ===\n")
...
cat("\n=== 方差齐性检验 (Levene's test) ===\n")
levene_result <- leveneTest(TPM ~ Temperature, data = data)
print(levene_result)

# Bartlett检验 (对正态分布数据)
cat("\n=== 方差齐性检验 (Bartlett's test) ===\n")
bartlett_result <- bartlett.test(TPM ~ Temperature, data = data)
print(bartlett_result)

# 由于数据不符合正态性和方差齐性假设，使用非参数检验
cat("\n=== 非参数检验：Kruskal-Wallis检验 ===\n")
kruskal_result <- kruskal.test(TPM ~ Temperature, data = data)
print(kruskal_result)

# 如果Kruskal-Wallis检验显著，进行事后检验
if(kruskal_result$p.value < 0.05) {
  cat("\n=== 非参数多重比较：Dunn检验 ===\n")
  dunn_result <- dunn.test(data$TPM, data$Temperature, method = "bonferroni")
  print(dunn_result)
