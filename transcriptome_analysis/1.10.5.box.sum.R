...
# 对残差进行正态性检验
temp_aov <- aov(total ~ Temperature, data = data_stats)
cat("\n=== 残差正态性检验 ===\n")
...
cat("\n=== 方差齐性检验 (Levene's test) ===\n")
levene_result <- leveneTest(total ~ Temperature, data = data_stats)
print(levene_result)

# Bartlett检验 (对正态分布数据)
cat("\n=== 方差齐性检验 (Bartlett's test) ===\n")
bartlett_result <- bartlett.test(total ~ Temperature, data = data_stats)
print(bartlett_result)

# 使用 Wilcoxon 秩和检验 (Mann-Whitney U test) - 适用于非正态分布数据
cat("\n=== Wilcoxon 秩和检验 (Mann-Whitney U test) ===\n")
wilcox_result <- wilcox.test(total ~ Temperature, data = data_stats, exact = FALSE)
print(wilcox_result)
cat("W 统计量:", wilcox_result$statistic, "\n")
cat("p 值:", wilcox_result$p.value, "\n")

# 根据 Wilcoxon 秩和检验结果确定显著性标记
...
p <- ggplot(data, aes(x = Temperature, y = total, fill = Temperature)) +
  ...
  geom_signif(comparisons = list(c("10", "16")),
              ...
              y_position = c(max(data$total) * 1.1),
              tip_length = 0.02,
              textsize = 6) +
  labs(
    y = "total/TPM",
