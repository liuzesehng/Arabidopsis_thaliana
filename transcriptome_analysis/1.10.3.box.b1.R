...
# 对残差进行正态性检验
temp_aov <- aov(β1.. ~ Temperature, data = data)
cat("\n=== 残差正态性检验 ===\n")
...
cat("\n=== 方差齐性检验 (Levene's test) ===\n")
levene_result <- leveneTest(β1.. ~ Temperature, data = data)
print(levene_result)

# Bartlett检验 (对正态分布数据)
cat("\n=== 方差齐性检验 (Bartlett's test) ===\n")
bartlett_result <- bartlett.test(β1.. ~ Temperature, data = data)
...
if (all(shapiro_results$shapiro_p > 0.05) && residuals_shapiro$p.value > 0.05 && levene_result$`Pr(>F)`[1] > 0.05) {
  ...
  anova_result <- aov(β1.. ~ Temperature, data = data)
  ...
  cat("\n=== Kruskal-Wallis 秩和检验 ===\n")
  kruskal_result <- kruskal.test(β1.. ~ Temperature, data = data)
  print(kruskal_result)
  cat("\n=== Dunn's 多重比较检验 ===\n")
  dunn_result <- dunn.test(data$β1.., data$Temperature, method="bonferroni")
...
if (all(shapiro_results2$shapiro_p > 0.05) && residuals_shapiro2$p.value > 0.05 && levene_result2$`Pr(>F)`[1] > 0.05) {
  ...
  cat("\n=== Kruskal-Wallis 秩和检验 ===\n")
  kruskal_result2 <- kruskal.test(β1 ~ Temperature, data = data)
  print(kruskal_result2)
  cat("\n=== Dunn's 多重比较检验 ===\n")
  dunn_result2 <- dunn.test(data$β1, data$Temperature, method="bonferroni")
