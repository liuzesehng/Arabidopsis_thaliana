...
# 创建箱线图
p <- ggplot(data, aes(x = Temperature, y = β_TPM.., fill = Temperature)) +
  # 添加箱须末端标记（上下边缘）
  ...
  # 设置y轴范围
  scale_y_continuous(limits = c(0, max(data$β_TPM.., na.rm = TRUE) * 1.4))
...
summary_stats <- data %>%
  ...
  summarise(
    count = n(),
    mean = mean(β_TPM.., na.rm = TRUE),
    median = median(β_TPM.., na.rm = TRUE),
    sd = sd(β_TPM.., na.rm = TRUE),
    min = min(β_TPM.., na.rm = TRUE),
    max = max(β_TPM.., na.rm = TRUE),
    q25 = quantile(β_TPM.., 0.25, na.rm = TRUE),
    q75 = quantile(β_TPM.., 0.75, na.rm = TRUE),
    .groups = 'drop'
  )
