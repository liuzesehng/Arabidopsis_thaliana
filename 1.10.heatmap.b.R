...
tpm_values <- data$β_TPM..
temperature <- data[, 1]

# 执行PCA（如果数据维度允许）
# 基于其他列创建更多特征用于PCA
feature_cols <- c("Tem..", "Lat", "Long", "alt.m", "β_TPM..")
...
# 创建热图数据 - 转置矩阵使样本成为列
heatmap_data <- matrix(data$β_TPM..[ordered_indices], nrow = 1)
colnames(heatmap_data) <- ordered_indices
rownames(heatmap_data) <- "β_TPM.."
...
# 保存为PDF
pdf_file <- "b_rca_pca_heatmap.pdf"
