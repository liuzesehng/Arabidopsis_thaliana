#!/bin/bash
# total Rca data
for file in $(ls salmon2/Abnormal/*/quant.genes.sf)
do
    filename=$(awk -F'/' '{print $3}' <<< $file)
    path=$(ls RCA2/Abnormal/*/*/*/$filename*txt)
    ...
    if [ $(echo "$persum2 > 50" | bc) -eq 1 ]; then
        if [ "$q" = "991" ]; then
            ...
            join -t $'\t' -1 1 -2 1 coex2/RCA.tran.txt coex2/$filename.tran.txt > temp && mv temp coex2/RCA.tran.txt
            rm -rf coex2/$filename.tran.txt
        fi
...
for file in $(ls salmon2/Normal/*/quant.genes.sf)
do
    filename=$(awk -F'/' '{print $3}' <<< $file)
