...
# 使用 pdf 保存图像
pdf("total/rca.a.cluster.pdf", height = 20, width = 30)  # 打开 PDF 设备
plot(sampleTree, main = "Sample clustering to detect outliers", sub = "", xlab = "")
dev.off()  # 关闭设备，保存文件
save(rca_total, file = "total/rca.a.Rdata")
...
# 创建PDF文件
pdf("total/a.soft_threshold_example.pdf", width = 10, height = 5)  # 设置宽度为10，高度为5
...
# 打开 PDF 设备 
pdf("total/a.module_dendrogram.pdf", height = 15, width = 15) 
# 绘制图形 
...
cyt = exportNetworkToCytoscape(
       modTOM,
      edgeFile = paste("total/a.CytoscapeInput-edges-", paste(modules, collapse="-"), ".txt", sep=""),
      nodeFile = paste("total/a.CytoscapeInput-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
      weighted = TRUE,
      threshold = 0.05,
      nodeNames = modProbes, 
...
tiff(file="total/a.METree. moduleColors.tiff",width=17,height=15, units="cm", compression="lzw", res=1200)
...
tiff(file="total/a.moduleColors.module_heatmap.tiff",width=17,height=17, units="cm", compression="lzw", res=1200)
...
#13.可视化-画模块之间的热图
tiff(file="total/a.moduleColors.Eigengene adjacency heatmap.tiff",width=17,height=21, units="cm", compression="lzw", res=1200)
...
write.table(datout, "total/a.module_gene_module.xls", sep="\t", row.names=F,quote=F)
...
sampleTree = hclust(dist(rca_total), method = "average")
# 使用 pdf 保存图像
pdf("total/rca.b.cluster.pdf", height = 20, width = 30)  # 打开 PDF 设备
plot(sampleTree, main = "Sample clustering to detect outliers", sub = "", xlab = "")
dev.off()  # 关闭设备，保存文件
save(rca_total, file = "total/rca.b.Rdata")
...
# 创建PDF文件
pdf("total/b.soft_threshold_example.pdf", width = 10, height = 5)  # 设置宽度为10，高度为5
...
cyt = exportNetworkToCytoscape(
       modTOM,
      edgeFile = paste("total/b.CytoscapeInput-edges-", paste(modules, collapse="-"), ".txt", sep=""),
      nodeFile = paste("total/b.CytoscapeInput-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
      weighted = TRUE,
      threshold = 0.05,
      nodeNames = modProbes, 
      nodeAttr = moduleColors[inModule])
...
MEDiss <- 1-cor(MEs)#距离矩阵
METree <- hclust(as.dist(MEDiss), method = "average")
tiff(file="total/b.METree. moduleColors.tiff",width=17,height=15, units="cm", compression="lzw", res=1200)
p1 <- plot(METree, main = "Clustering of module eigengenes", xlab = "", sub = "")
print(p1)
dev.off()
...
#12.可视化-画模块之间的热图
dissTOM = 1-TOMsimilarityFromExpr(rca_total, power = 14);
tiff(file="total/b.moduleColors.module_heatmap.tiff",width=17,height=17, units="cm", compression="lzw", res=1200)
plotTOM <- dissTOM^7##为了更显著，用7次方
p1 <- TOMplot(plotTOM, geneTree, moduleColors, main="Network heatmap plot, all genes")
print(p1)
dev.off()
...
#13.可视化-画模块之间的热图
tiff(file="total/b.moduleColors.Eigengene adjacency heatmap.tiff",width=17,height=21, units="cm", compression="lzw", res=1200)
p2 <- plotEigengeneNetworks(MEs, 
                      "Eigengene adjacency heatmap", 
                      marHeatmap = c(4,4,4,4), 
                     plotDendrograms = T, 
...
datout=data.frame(datSummary,colorNEW=color1,datKME )
#write.table(datout, "pink_gene_module.xls", sep="\t", row.names=F,quote=F)
write.table(datout, "total/b.module_gene_module.xls", sep="\t", row.names=F,quote=F)
